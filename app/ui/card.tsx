...
export function Card({title, subtitle = "EMPTY", icon = undefined, status, visitRef, detailsRef}: {
    title: string;
    subtitle: string;
    status: number;
    icon: any;
    ...
    return (
        <div
            class="flex flex-col justify-center items-center gap-2 border border-dashed border-gray-500 p-4 rounded-md h-24">
            <div class="flex gap-2 items-center">
                {visitRef != undefined ?
                    <a className="font-bold text-xl underline hover:text-blue-600" href={visitRef}>{title}</a>
                    :
                    <p className="font-bold text-xl" href={visitRef}>{title}</p>
                }
                {CardIcon}
            </div>
            <span class={clsx("font-semibold opacity-70 text-sm text-center", {
