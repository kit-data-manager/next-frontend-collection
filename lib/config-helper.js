import config from "@/config/search/indices.json";
...
function buildDateRangeFacetFromConfig(facetConfig, accumulator) {
    ...
    if (facetConfig["type"] === "date_year") {
        ...
        ranges = [
            {
                from: moment().month("January").date(1).hour(0).minute(0).second(0).format("YYYYMMDDTHH:mm:ss"),
                to: moment().month("December").date(31).hour(23).minute(59).second(59).format("YYYYMMDDTHH:mm:ss"),
                name: "This Year"
            },
            {
